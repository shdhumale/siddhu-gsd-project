...
- `delete <slug>` — Permanently delete an active debug session
</objective>
...
- If $ARGUMENTS starts with "delete ": SUBCMD=delete, SLUG=remainder (trim whitespace)
...
## 1d. DELETE subcommand

When SUBCMD=delete and SLUG is set:

Check `.planning/debug/{SLUG}.md` exists. If not:
- Check `.planning/debug/resolved/{SLUG}.md` if exists: print "Cannot delete resolved session: {SLUG}. Resolved sessions are archived." and stop.
- Otherwise print "No active debug session found with slug: {SLUG}. Check `/gsd-debug list` for active sessions." and stop.

Before deletion confirm with user:
```
⚠️  WARNING: This will permanently delete the debug session: {SLUG}
This action cannot be undone. Are you sure you want to delete this session? [y/N]
```

If user confirms with 'y' or 'yes':
```bash
rm .planning/debug/{SLUG}.md
```

Print confirmation message:
```
✅ Debug session '{SLUG}' has been deleted successfully.
```

No agent spawn. STOP after deletion.
